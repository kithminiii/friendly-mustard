...
export const loadCommonRequireMock = (jest: Record<any, any>, testConfig: Record<any, any>): void => {
    ...
    jest.mock(resolve(process.cwd(), 'src/services/stripe-service/stripe-service'), () => ({
        updateProjectQuantity: jest.fn()
    }));
};

export const mockMiddleware = (_req: expressType.Request, _res: expressType.Response, next: expressType.NextFunction): void => {
    next();
};
