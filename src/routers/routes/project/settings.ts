...
// enum CUSTOM_DOMAIN_CDN_VALIDATION_TYPES {
//     NO_CNAME = 'NO_CNAME',
...
export async function updateCustomDomain(req: expressType.Request, res: expressType.Response): Promise<any> {
    try {
        ...
        if (!project.checkTierAllowanceForFeature('builtInCustomDomain')) {
        ...
        // replace by
        // dns has generic resolvers that we can just check to see if there are any mappings to either cname/ip/netlify load balanacer
