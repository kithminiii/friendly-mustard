...
function BackupMongoDatabase(config) {
    return new Promise((resolve, reject) => {
        ...
        let command = uri
            ? `mongodump --uri="${uri}" --gzip --archive=${BACKUP_PATH(DB_BACKUP_NAME)}`
...
function UploadBackup(config, backupResult) {
    ...
    return UploadFileToS3(s3, backupResult.zipName, config).then(
        (uploadFileResult) => {
...
function CreateBackup(config) {
    ...
    return BackupMongoDatabase(config).then(
        (result) => {
...
function BackupAndUpload(config) {
    ...
    return CreateBackup(config).then(
        (backupResult) => {
            logger.debug('DB Backup mongodump created');
            // Upload it to S3
            return UploadBackup(config, backupResult).then(
                (res) => {
...
async function getBackups(options) {
    const s3 = new AWS.S3();
    const listParams = {
    ...
    const backups = (await s3.listObjects(listParams).promise()).Contents;
