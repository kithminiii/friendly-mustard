...
async function handleUnhealthy(project: IProjectDoc, user: IUserDoc | undefined, environmentName?: string | null, isFatal?: boolean) {
    ...
    const fatalWhenStarting =
        isFatal && project.getDeploymentData<string>('container.status', environmentName) === ContainerStates.starting;
    ...
    if (fatalWhenStarting || newTaskCreatedAt) {
        ...
        const timeSinceUpgradeStart =
            new Date().getTime() - project.getDeploymentData<Date>('container.newTaskCreatedAt', environmentName, new Date()).getTime();
        ...
        if (shouldHibernate) {
            ...
            const updatedProject = await Project.updateDeploymentData(
                project._id!,
                'container',
                {
            ...
            analytics.track(
                'SharedContainer Project Upgrade Failed',
                {
                    projectId: project.id,
                    userId: user?.id
                },
    ...
    analytics.track(
        'SharedContainer Project Preview Unhealthy',
        {
            projectId: project.id,
            userId: user?.id,
            isFatal
    ...
    if (isFatal || task.healthy === false || taskCrashed) {
        ...
        analytics.track(
            'Error: SharedContainer Project Preview Unhealthy - Redeploying',
            {
                projectId: project.id,
                userId: user?.id
            },
...
export async function upgrade(project: IProjectDoc, buildLogger?: winston.Logger): Promise<void> {
    await Promise.all(
        getProjectEnvironments(project).map((environmentName) => {
            return upgradeContainer(project, environmentName, buildLogger);
        })
    );
}

export async function upgradeIfNeeded(project: IProjectDoc, buildLogger?: winston.Logger): Promise<void> {
    const service = getProjectPreviewContainerService(project);
    await Promise.all(
        getProjectEnvironments(project).map(async (environmentName) => {
    ...
    if (data.action === 'lifecycleEvent') {
        ...
        if (data.event === 'initialized') {
            ...
            if (
                ...
                (project.getDeploymentData<boolean>('container.healthy', environmentName) === null || !taskHandle || !newTaskHandle)
            ) {
        ...
        if (project.getDeploymentData<boolean>('container.healthy') === null && data.healthy) {
            analytics.track(
                'SharedContainer Project Preview Live',
                {
                    projectId: project.id,
                    userId: user?.id
                },
        ...
        if (data.healthy) {
            ...
            setTimeout(async () => {
                try {
                    ...
                    logger.error('[shared-container] error handling unhealthy', {
                        projectId: project.id,
                        err,
...
export async function hibernateContainer(project: IProjectDoc, user: IUserDoc | undefined, environmentName?: string | null): Promise<void> {
    ...
    await Promise.all(
        getRoutedUrls(project, environmentName).map((host) => {
...
async function checkInactivity(project: IProjectDoc, user: IUserDoc | undefined) {
    ...
    const shouldHibernate = project.shouldHibernate ?? true;
    const healthy = project.getDeploymentData('container.healthy');
    const lastActivityDate = _.max(
        allContainerEnvironments.map((environmentName) => {
    ...
    if (shouldHibernate && lastActivityDate && Date.now() - lastActivityDate.getTime() > containerMaxInactivityTime && healthy !== null) {
        logger.debug('[shared-container] container inactive, shutting down', {
            lastActivityDate,
...
export async function deleteContainer(
    ...
    try {
        ...
        if (shouldUnregisterUrl) {
            await Promise.all(
                getRoutedUrls(project, environmentName).map(async (host) => {
        ...
        const prevTaskHandle =
            project.getDeploymentData(`container.${service.taskHandleKey}`, environmentName) ??
...
export async function initializeContainerEnvironments(): Promise<void> {
...
export async function checkHealth(
    ...
    try {
        ...
        if (result?.status === 'ok' && project.getDeploymentData<string>('container.status', environmentName) !== ContainerStates.running) {
            await Project.updateDeploymentData(
                project._id!,
                'container',
                {
