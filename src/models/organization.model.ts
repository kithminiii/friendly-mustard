...
OrganizationSchema.statics.createOrganization = async function (organization, creatorId) {
    ...
    let savedOrganization = await new Organization(newOrganization).save();

    if (creatorId) {
        // grant admin role to creator
        savedOrganization = await Organization.setUserRole(savedOrganization.id!, ORGANIZATION_ROLES.ORG_FULL_ADMIN, creatorId);
    }
