...
ProjectSchema.typeSafeVirtual('containerMaxInactivityTimeInMinutes', function () {
    const useExtendedInactivityTime =
        Date.now() - this.createdAt.getTime() <
        (this.availableFeatures.containerInitialExtendedHibernationPeriodInMinutes ?? 0) * 60 * 1000;
    if (useExtendedInactivityTime) {
        return this.availableFeatures.containerExtendedMaxInactivityTimeInMinutes;
    } else {
...
ProjectSchema.methods.addRequestedPublish = async function (requester, date, text) {
    this.requestedPublishes = this.requestedPublishes.filter((req) => req.requester?.toString() !== requester._id?.toString());
